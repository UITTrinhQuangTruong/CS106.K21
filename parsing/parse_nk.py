...
# %%
def get_xlmr(hparams):
    from fairseq.models.roberta import XLMRModel
    if hparams['bert_model'] == 'xlmr.base':
        bert = XLMRModel.from_pretrained('/content/gdrive/My Drive/DeadlineAI/src/xlmr.base', checkpoint_file='model.pt')
    elif hparams['bert_model'] == 'xlmr.large':
        bert = XLMRModel.from_pretrained('/content/gdrive/My Drive/DeadlineAI/src/xlmr.large', checkpoint_file='model.pt')
    if use_cuda: bert.cuda()
    bert.eval()
    return bert
    
    
def get_phobert(hparams):
    ...
    vocab = Dictionary()
    if hparams['bert_model'] == 'PhoBERT.base':
        vocab.add_from_file("PhoBERT.base/dict.txt")
    elif hparams['bert_model'] == 'PhoBERT.large':
        vocab.add_from_file("PhoBERT.large/dict.txt")


    parser = argparse.ArgumentParser()
    if hparams['bert_model'] == 'PhoBERT.base':
        parser.add_argument('--bpe-codes',  default="PhoBERT.base/bpe.codes", required=False, type=str, help='path to fastBPE BPE')
    elif hparams['bert_model'] == 'PhoBERT.large':
        parser.add_argument('--bpe-codes',  default="PhoBERT.large/bpe.codes", required=False, type=str, help='path to fastBPE BPE')

    args = parser.parse_args([])
    bpe = fastBPE(args)
    if hparams['bert_model'] == 'PhoBERT.base':
        config = RobertaConfig.from_pretrained("PhoBERT.base/config.json")
        bert = RobertaModel.from_pretrained("PhoBERT.base/model.bin", config=config)
    elif hparams['bert_model'] == 'PhoBERT.large':
        config = RobertaConfig.from_pretrained("PhoBERT.large/config.json")
        bert = RobertaModel.from_pretrained("PhoBERT.large/model.bin", config=config)
    return vocab, bpe, bert
# %%
...
class NKChartParser(nn.Module):
    ...
    def __init__(
        ...
        if hparams.use_chars_lstm:
            ...
            if self.spec['hparams']['bert_model'] in ['xlmr.base', 'xlmr.large']:
                self.bert = get_xlmr(self.spec['hparams'])
                self.d_bert_annotations = self.bert.args.encoder_embed_dim
                self.bert_max_len = self.bert.args.max_positions
            elif self.spec['hparams']['bert_model'] in ['PhoBERT.base', 'PhoBERT.large']:
                self.vocab_bert, self.bpe, self.bert = get_phobert(self.spec['hparams'])
                self.bert_max_len = self.bert.embeddings.position_embeddings.num_embeddings
                self.d_bert_annotations = self.bert.pooler.dense.in_features

            if hparams.use_bert_only:
                self.project_bert = nn.Linear(self.d_bert_annotations, hparams.d_model, bias=False)
            else:
                self.project_bert = nn.Linear(self.d_bert_annotations, self.d_content, bias=False)
    ...
    def parse_batch(self, sentences, golds=None, return_label_scores_charts=False):
        ...
        if self.char_encoder is not None:
        ...
        elif self.bert is not None and self.spec['hparams']['bert_model'] in ['xlmr.base', 'xlmr.large']:
            all_sentence = [' '.join([BERT_TOKEN_MAPPING.get(w[1], w[1]) for w in sentence]) for sentence in sentences]
            batch = collate_tokens([self.bert.encode(s) for s in all_sentence], pad_idx=1)
            ...
            for snum, sentence in enumerate(sentences):
                ...
                length = 1
                bert_lens.append(1)
                for row, tk in enumerate([BERT_TOKEN_MAPPING.get(w[1], w[1]) for w in sentence]):
                    sw_len = len(self.bert.encode(tk)[1:-1])
                    for _ in range(sw_len):
                        word_start_mask.append(0)
                        word_end_mask.append(0)
                        
                    word_start_mask[length] = 1
                    word_end_mask[-1] = 1
                    length += sw_len
                    bert_lens.append(sw_len)

                word_start_mask.append(1)
                word_end_mask.append(1)
                bert_lens.append(1)
                length += 1
                all_input_mask[snum, :length] = [1] * (length)
                all_word_start_mask[snum, :len(word_start_mask)] = word_start_mask
                all_word_end_mask[snum, :len(word_end_mask)] = word_end_mask
                subword_max_len = max(subword_max_len, length)
            ...
            if self.encoder is not None:
                #extra_content_annotations = self.project_bert(torch.stack([i.mean(0) for i in self.bert.extract_features(batch).masked_select(all_input_mask.to(torch.bool).unsqueeze(-1)).reshape(-1, self.d_bert_annotations).split(bert_lens)]))
                extra_content_annotations = self.project_bert(self.bert.extract_features(batch).masked_select(all_word_start_mask.to(torch.bool).unsqueeze(-1)).reshape(-1, self.d_bert_annotations))
            else:
                features = self.bert.extract_features(batch).masked_select(all_input_mask.to(torch.bool).unsqueeze(-1)).reshape(-1, self.d_bert_annotations)

                
        elif self.bert is not None and self.spec['hparams']['bert_model'] in ['PhoBERT.base', 'PhoBERT.large']:
            ...
            for snum, sentence in enumerate(sentences):
                ...
                input_ids = self.vocab_bert.encode_line("<s>", append_eos=False, add_if_not_exist=False).long().tolist()
                for word in cleaned_words:
                    subwords = self.bpe.encode(word)
                    idxs = self.vocab_bert.encode_line(subwords, append_eos=False, add_if_not_exist=False).long().tolist()
                    for _ in range(len(idxs)):
                        word_start_mask.append(0)
                        word_end_mask.append(0)
                    word_start_mask[len(input_ids)] = 1
                    word_end_mask[-1] = 1
                    input_ids.extend(idxs)
                    
                input_ids = input_ids + self.vocab_bert.encode_line("</s>", append_eos=False, add_if_not_exist=False).long().tolist()
            ...
            if self.encoder is not None:
                extra_content_annotations = self.project_bert(self.bert(all_input_ids, attention_mask=all_input_mask)[0].masked_select(all_word_start_mask.to(torch.bool).unsqueeze(-1)).reshape(-1, self.d_bert_annotations))
            else:
