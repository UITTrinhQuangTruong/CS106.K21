...
import gzip
import copy
delete=['(',')','?','NP','QP','VP','ADJP','PP','RP','CONJP','UCP','QNP','QRP',
'QPP','QADJP','MDP','SQ','SPL','SBAR','XP','*T*','*E*','*','*0*','*P*','H','D','Vv_swsp_Rt','Aa_swsp_Rt','Nn_swsp_Rt','On_swsp_Rt','Nn_swsp','Nn_swsp','Nn_swsp','R_w','Aa_w','Vv_w','Nn_w',
'Sv','Nc','Ncs','Nun','Nw','Num','Nu','Nq','Nr','Nn','Ve','Vc','S','VA','VN','NA','Vcp','Vv','An','v','Aa','Pd','Pp','R','Cs','Cp','ON','ID','E','M','FW','X','PU']


transform={0:'khái niệm',
            1:'liệt kê',
            2:'cách cài đặt'}
...
def run_train(args, hparams):
    ...
    load_path = "/content/gdrive/My Drive/self-attentive-parser-final/models/niivtb_1_xlmrlarge_dev=85.96.pt"
...
def run_test(args):
    ...
    with open("data/NIIVTB_bert_large.txt","w",encoding="UTF-8") as f:
        for pred in test_predicted:
            print(pred.linearize())
    ...
    with open('./prediction.txt', 'w', encoding='UTF-8') as f:
        print('\n'.join([p.linearize() for p in test_predicted]), file=f)
...
#%%
def searchkey(node, keyword=deque()):
    if isinstance(node, InternalParseNode):
        print(node.label)
        if "NP" in node.label or "VP" in node.label :
            keyword.appendleft(node.convert().linearize())
            if(len(keyword)>3):
               keyword.pop()
        for child in node.children:
            searchkey(child, keyword)

def search_SQ(node, keyword=deque()):
    if isinstance(node, InternalParseNode):
        if "SQ" in node.label:
            print(node.label)
            keyword.appendleft(node.convert().linearize())
...
def run_parse(args):
    ...
    tokenize.fit_on_texts(query)  
    X = tokenize.texts_to_matrix(X)
    test=tokenize.texts_to_matrix(query)  
    ...
    pred=np.argmax(loaded_model.predict(test)[0])
    all_predicted = []
    ...
    if args.output_path != '-':
        ...
        print("Tree parse written to:", args.output_path)
    query=load_trees(args.output_path)
    keyword =deque()
    print(query[0].linearize())
    searchkey(query[0].convert(),keyword)
    #search_SQ(query[0].convert(),sq)
    k_word=list(keyword)
    #sq=list(sq)
    print("key:",k_word)
    #print("sq:",sq)
    ...
    with open(args.output_path, 'w') as output_file:
        ...
        output_file.write(transform[pred])
...
def main():
    ...
    subparser.add_argument("--train-path", default="data/NIIVTB_1/bracket_train.txt")
    subparser.add_argument("--dev-path", default="data/NIIVTB_1/bracket_dev.txt")
    ...
    subparser.add_argument("--model-path-base", required=True)
    subparser.add_argument("--evalb-dir", default="EVALB/")
    subparser.add_argument("--test-path", default="data/NIIVTB_1/bracket_test.txt")
    ...
    subparser.add_argument("--evalb-dir", default="EVALB/")
    subparser.add_argument("--test-path", default="data/VTB_test_word.txt")
    subparser.add_argument("--eval-batch-size", type=int, default=100)
    ...
    subparser.add_argument("--model-path-base", required=True)
    subparser.add_argument("--evalb-dir", default="EVALB/")
    subparser.add_argument("--viz-path", default="data/VTB_test_word.txt")
    subparser.add_argument("--eval-batch-size", type=int, default=100)
